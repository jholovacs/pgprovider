...
namespace pgProvider
{
	public class pgMembershipProvider : MembershipProvider
	{
		...
		public pgMembershipProvider() : this(LogManager.GetLogger<pgMembershipProvider>())
		{
		...
		public override void Initialize(string name, NameValueCollection config)
		{
			...
			{
				var message = string.Format("Error initializing the membership configuration settings: {0}", ex);
		...
		public void ChangePassword(string username, string newPassword)
		{
			...
			using (var conn = new NpgsqlConnection(ConnectionString))
			{
				...
				using (var comm = new NpgsqlCommand("update_user_password", conn))
				{
					...
					comm.Parameters.Add("_salt", NpgsqlDbType.Varchar, 250).Value = salt;
					comm.Parameters.Add("_password", NpgsqlDbType.Bytea).Value = hash;
					comm.ExecuteNonQuery();
				}
			}
		...
		{
			_logger.Debug(
				d => d("ChangePasswordQuestionAndAnswer(string, string, string, string) for '{0}' beginning...", username));
		...
		protected void UpdateQAndA(string username, string question, string answer)
		{
			...
			catch (Exception ex)
			{
				...
				{
					_logger.Error("Questions and Answers are required to be provided; but they are not properly supplied.", ex);
					throw ex;
				}
			...
			out int totalRecords)
		{
			...
			using (var conn = new NpgsqlConnection(ConnectionString))
			{
				...
				{
					comm.CommandType = CommandType.StoredProcedure;
					comm.Parameters.Add("partial_email", NpgsqlDbType.Varchar, 250).Value = emailToMatch;
					comm.Parameters.Add("_application_name", NpgsqlDbType.Varchar, 250).Value = _ApplicationName;
			...
			out int totalRecords)
		{
			...
			using (var conn = new NpgsqlConnection(ConnectionString))
			{
				...
				{
					comm.CommandType = CommandType.StoredProcedure;
					comm.Parameters.Add("partial_username", NpgsqlDbType.Varchar, 250).Value = usernameToMatch;
					comm.Parameters.Add("_application_name", NpgsqlDbType.Varchar, 250).Value = _ApplicationName;
					using (var reader = comm.ExecuteReader())
					{
						var users = GetUsersFromReader(reader);
		...
		public override string ResetPassword(string username, string answer)
		{
			...
			var newPass = Membership.GeneratePassword(MinRequiredPasswordLength, MinRequiredNonAlphanumericCharacters);
			ChangePassword(username, newPass);
			_logger.Info(i => i("User '{0}' has reset their password.", username));
			return newPass;
		}
		...
		public override bool ValidateUser(string username, string password)
		{
			...
			{
				_logger.Warn(w => w("Username is null or whitespace."));
				return false;
			}
					...
					using (IDataReader dr = comm.ExecuteReader())
					{
						...
						if (dr.Read())
						{
							return new CredentialPackage
							{
								...
								PasswordHash = dr.IsDBNull(passwordColumn)
									? new byte[]
									{}
									: (byte[]) dr.GetValue(passwordColumn),
								AnswerSalt = dr.IsDBNull(answerSaltColumn) ? string.Empty : dr.GetString(answerSaltColumn),
								AnswerHash = dr.IsDBNull(passwordAnswerColumn)
									? new byte[]
									{}
		...
		protected MembershipUserCollection PaginateUserCollection(MembershipUserCollection source, int pageIndex, int pageSize,
			out int totalRecords)
		{
		...
		public void PurgeActivity(long secondsOld)
		{
			using (var conn = new NpgsqlConnection(ConnectionString))
			{
				...
				{
					comm.CommandType = CommandType.StoredProcedure;
					comm.Parameters.Add("olderthan", NpgsqlDbType.Bigint).Value = secondsOld;
					comm.ExecuteNonQuery();
				}
			}
		}
