...
namespace pgProvider
{
	public class pgMembershipProvider : MembershipProvider
	{
		...
		public override void Initialize(string name, NameValueCollection config)
		{
			...
			try
			{
				...
				if (_EnablePasswordRetrieval)
				{
					...
					{
						throw new ProviderConfigurationException("When password retrieval is enabled, a Base-64 encoded 32-byte encryption key must be supplied.");
					}
				...
				{
					throw new ProviderConfigurationException("MaxSaltCharacters value cannot be less than MinSaltCharacters value.");
				}
				#endregion
		...
		protected virtual void ValidatePasswordComplexity(string newPassword)
		{
			...
			if ((newPassword.Length - OnlyTheAlphanumericLowercase(newPassword).Length) < _MinRequiredNonAlphanumericCharacters)
			{
				throw new PasswordComplexityException(string.Format("Password requires a minimum of {0} non-alphanumeric characters.", _MinRequiredNonAlphanumericCharacters));
			}
			if (!string.IsNullOrEmpty(_PasswordStrengthRegularExpression) &&
				(!System.Text.RegularExpressions.Regex.IsMatch(newPassword, _PasswordStrengthRegularExpression)))
			{
		...
		{
			Log.Debug(d => d("ChangePasswordQuestionAndAnswer(string, string, string, string) for '{0}' beginning...", username));
		...
		protected void UpdateQAndA(string username, string question, string answer)
		{
			...
			catch (Exception ex)
			{
				...
				{
					Log.Error("Questions and Answers are required to be provided; but they are not properly supplied.", ex);
					throw ex;
				}

				Log.Warn(w => w("Questions and Answers are enabled, but they have not been properly provided.  No updates to the Questions and Answers will be performed at this time.", ex));
			}
		...
		protected string OnlyTheAlphanumericLowercase(string original)
		{
			...
			var cleanedUp = System.Text.RegularExpressions.Regex.Replace(original, @"[^a-zA-Z0-9]", string.Empty);
		...
		public override string GetPassword(string username, string answer)
		{
			...
			{
				throw new InvalidOperationException("GetPassword() is only valid if the passwords are stored in Encrypted form in the database.");
			}
		...
		public override string GetUserNameByEmail(string email)
		{
			...
			if (!_RequiresUniqueEmail) throw new InvalidOperationException("Cannot use GetUserNameByEmail() unless the RequiresUniqueEmail attribute is set to 'true'.");
		...
		public override string ResetPassword(string username, string answer)
		{
			...
			Log.Info(i => i("User '{0}' has reset their password.  The supplied password has bypassed complexity requirements.", username));
			return newPass;
		}
					...
					using (IDataReader dr = comm.ExecuteReader())
					{
						...
						if (dr.Read())
						{
							return new CredentialPackage()
							{
								...
								PasswordHash = dr.IsDBNull(passwordColumn) ? new byte[] { } : (byte[])dr.GetValue(passwordColumn),
								AnswerSalt = dr.IsDBNull(answerSaltColumn) ? string.Empty : dr.GetString(answerSaltColumn),
								AnswerHash = dr.IsDBNull(passwordAnswerColumn) ? new byte[] { } : (byte[])dr.GetValue(passwordAnswerColumn),
								LockedOutAsOf = dr.IsDBNull(lastLockoutColumn) ? DateTime.MinValue : dr.GetDateTime(lastLockoutColumn)
							};
						}
						return null;
					}
		...
		public override string Description
		{
			get
			{
				return "PostgreSQL ASP.Net MembershipProvider class";
			}
		}
